...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (optimizeRouteBtn && !optimizeRouteBtn.hasAttribute('data-listener-added')) {
        optimizeRouteBtn.setAttribute('data-listener-added', 'true'); // Previne duplicação
        ...
        optimizeRouteBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok && result.routeId) {
                    ...
                    // 🎯 CÁLCULO DE TEMPOS (com delay para garantir que o mapa está pronto)
                    setTimeout(async () => {
                        try {
                            if (typeof calculateAndDisplayDeliveryTimes === 'function') {
...
function renderDeliveryItemContentWithTimes(item, index) {
    if (item.type === 'pickup') {
        ...
        return `
            ...
            <!-- O tempo estimado será inserido aqui dinamicamente -->
            ${orderNumberDisplay}
            <p><strong>📍 Endereço:</strong> ${item.address}</p>
            <p><strong>📦 Produto:</strong> ${item.product_description}</p>
            ${item.customer_phone ? `<p><strong>📞 Telefone:</strong> ${item.customer_phone}</p>` : ''}
            <div class="delivery-actions">
                ...
                <button onclick="showDeliveryOnMap(${parseFloat(item.lat)}, ${parseFloat(item.lng)})" class="btn btn-secondary btn-sm">🗺️ Mapa</button>
                <button onclick="generateTrackingLink('${item.id}')" class="btn btn-info btn-sm">🔗 Link</button>
                ${item.status === 'in_transit' ? `<button onclick="completeDelivery('${item.id}')" class="btn btn-success btn-sm">✅ Entregar</button>` : ''}
...
window.showOptimizedRoute = function(route) {
    // Chama a função original
    if (originalShowOptimizedRoute) {
        originalShowOptimizedRoute.call(this, route);
    }
    
    // Adiciona funcionalidade de toggle do botão
    if (typeof toggleTimesButton === 'function') {
        toggleTimesButton();
    }
};
